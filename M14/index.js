...
// CREATE TABLE accounts ( id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL , createdAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updatedAt TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP , token VARCHAR(255) DEFAULT NULL );
...
app.get("/register", (req, res) => {
  // Kirim role Super Admin ke form registrasi
  res.render("register", { message: "", role: req.session.account?.role });
});
...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    if (existingUser) {
      return res.render("register", { message: "Email already registered", role: req.session.account?.role });
    }

    // Jika Super Admin yang login, boleh memilih Admin
    if (req.session.account?.role !== "Super Admin" && role === "Admin") {
      return res.render("register", { message: "You are not authorized to create Admins.", role: req.session.account?.role });
    }

    await Account.create({ username, email, password, role: role || "User" }); // Default role User
    res.redirect("/login");
  } catch (error) {
    res.status(500).render("register", { message: "Registration failed", role: req.session.account?.role });
  }
});
...
app.get("/create", async (req, res) => {
  try {
    ...
    res.render("create", { account, categories });
  } catch (error) {
    res.status(500).json({ status: 500, message: error.message });
  }
});
...
app.get("/edit/:id", async (req, res) => {
  try {
    ...
    res.render("edit", { note, categories, account: req.session.account });
  } catch (error) {
    res.status(500).json({ status: 500, message: error.message });
  }
});
