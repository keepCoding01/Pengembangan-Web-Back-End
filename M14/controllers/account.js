...
const auth = async (req, res) => {
  ...
  try {
    ...
    const tokenPayload = { id: user.id, email: user.email, role: user.role }; // Menyertakan role
    let token;
    if (user.token) {
      token = user.token;
    } else {
    ...
    req.session.account = { id: user.id, email: user.email, username: user.username, role: user.role }; // Menyimpan role di session
    res.redirect("/");
  } catch (error) {
