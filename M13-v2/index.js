...
app.get("/", (req, res) => {
  Supplier.findAll().then((results) => {
    res.render("index", { suppliers: results, account: req.session.account || "" });
  });
});

app.get("/logIndex", (req, res) => {
  Supplier.findAll().then((results) => {
    res.render("logIndex", { suppliers: results, account: req.session.account || "" });
  });
});
...
app.post("/api/suppliers", (req, res) => {
  Supplier.create({ company_name: req.body.company_name, contact_name: req.body.contact_name, email: req.body.email, phone: req.body.phone, createdBy: req.session.account.username })
    .then((results) => {
...
app.put("/api/supplier/:id", (req, res) => {
  Supplier.update(
    { company_name: req.body.company_name, contact_name: req.body.contact_name, email: req.body.email, phone: req.body.phone, active: req.body.active, updatedBy: req.session.account.username },
